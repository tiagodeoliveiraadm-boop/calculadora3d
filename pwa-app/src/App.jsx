...
import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut, onAuthStateChanged, updatePassword, EmailAuthProvider, reauthenticateWithCredential } from "firebase/auth";
import { getFirestore, doc, setDoc, getDoc } from "firebase/firestore";

const firebaseConfig = {
  apiKey: "AIzaSyAGwBEXqDOSgrz8mEhfUblgANJJes8fiTY",
  authDomain: "calculadora-casa-oliver.firebaseapp.com",
  projectId: "calculadora-casa-oliver",
  storageBucket: "calculadora-casa-oliver.firebasestorage.app",
  messagingSenderId: "1035627317891",
  appId: "1:1035627317891:web:9b066c899dfba03d2e29b6",
  measurementId: "G-TQ4BG27JXN"
};
...
const fsGet = async (col, id) => {
  try { const s = await getDoc(doc(firestore, col, id)); return s.exists() ? s.data() : null; } catch { return null; }
};
const fsSet = async (col, id, data) => {
  try { await setDoc(doc(firestore, col, id), data, { merge: true }); } catch(e) { console.error(e); }
};
...
// ─── AUTH / STORAGE ───────────────────────────────────────────────────────────
// Firebase Auth handles password hashing natively
// Firebase-backed db — replaces localStorage
const db = {
  // Auth handled by Firebase Auth directly
  async getProfile(uid)    { return fsGet("profiles", uid); },
  async setProfile(uid, d) { return fsSet("profiles", uid, d); },
  async getProducts(uid)   { const r = await fsGet("products", uid); return r?.list || []; },
  async setProducts(uid,d) { return fsSet("products", uid, { list: d }); },
  async getTemplates(uid)  { const r = await fsGet("templates", uid); return r?.list || []; },
  async setTemplates(uid,d){ return fsSet("templates", uid, { list: d }); },
  async getQuoteHistory(uid)   { const r = await fsGet("quotes", uid); return r?.list || []; },
  async setQuoteHistory(uid,d) { return fsSet("quotes", uid, { list: d }); },
};
...
export default function App() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (fbUser) => {
      if (fbUser) {
        const u = { uid: fbUser.uid, email: fbUser.email, name: fbUser.displayName || fbUser.email.split("@")[0] };
        setUser(u);
        loadAllData(u.uid);
      } else {
        setUser(null);
  ...
  const loadAllData = async (uid) => {
    const p=await db.getProfile(uid); if(p)applyProfile(p);
    const prods=await db.getProducts(uid); setSavedProducts(prods);
    const tmpl=await db.getTemplates(uid); setTemplates(tmpl);
    const hist=await db.getQuoteHistory(uid); setQuoteHistory(hist);
  };
  ...
  const handleAuthSubmit = async () => {
    ...
    try {
      ...
      if (authMode === "reset") {
        ...
        if (!fbUser) { setAuthError("Faça login primeiro para redefinir a senha."); return; }
        const cred = EmailAuthProvider.credential(fbUser.email, authConfirm); // old pass in confirm field
        await reauthenticateWithCredential(fbUser, cred);
        await updatePassword(fbUser, authPass);
        setSaveMsg("✅ Senha redefinida!"); setTimeout(() => setSaveMsg(""), 3000);
        setAuthMode("login"); setAuthPass(""); setAuthConfirm("");
        setShowAuthPanel(false); return;
      }
      ...
      if (authMode === "register") {
        ...
        // Save display name in Firestore profile
        await fsSet("profiles", cred.user.uid, { name: authName, email: authEmail, createdAt: Date.now() });
      ...
      const msg = e.code === "auth/email-already-in-use" ? "E-mail já cadastrado."
        : e.code === "auth/user-not-found" || e.code === "auth/wrong-password" || e.code === "auth/invalid-credential" ? "E-mail ou senha incorretos."
        : e.code === "auth/too-many-requests" ? "Muitas tentativas. Aguarde um momento."
  ...
  const getCurrentProfile = () => ({ name:user?.name||"", email:user?.email||"", material,filamentWeight,filamentPrice,wasteFactor,printHours,printMinutes,printerModel,customWatts,kwhPrice,printerCost,printerLifespan,prepHours,laborRate,failRate,profitMargin,taxRate,addonStates,customAddons,companyName:savedCompanyName,companyContact:savedCompanyContact,savedAt:Date.now() });
  const saveProfile = async () => {
    if(!user)return;
    await db.setProfile(user.uid, getCurrentProfile());
  ...
  const saveProduct = async () => {
    ...
    if(user) await db.setProducts(user.uid,updated);
    setProductName(""); setSaveMsg("✅ Produto salvo!"); setTimeout(()=>setSaveMsg(""),2500);
  };
  const deleteProduct = async (id) => { const u=savedProducts.filter(p=>p.id!==id); setSavedProducts(u); if(user)await db.setProducts(user.uid,u); };
  ...
  const saveTemplate = async () => {
    ...
    if(user)await db.setTemplates(user.uid,updated);
  ...
  const deleteTemplate = async (id) => { const u=templates.filter(t=>t.id!==id); setTemplates(u); if(user)await db.setTemplates(user.uid,u); };
  ...
  const saveQuoteToHistory = async (quoteObj) => {
    ...
    if(user)await db.setQuoteHistory(user.uid,updated);
  };
  const deleteQuote = async (id) => { const u=quoteHistory.filter(q=>q.id!==id); setQuoteHistory(u); if(user)await db.setQuoteHistory(user.uid,u); };
